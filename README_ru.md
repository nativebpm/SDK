...
## ⚙️ Разработка SDK, изменение API и генерация кода в монорепозитории
...
### 1. Как работать с SDK

SDK в данном монорепозитории имеют двухслойную архитектуру (при этом для Go также предоставляется генерация серверного контрактного слоя):
1. **Низкоуровневый сгенерированный клиент**: Автоматически генерируется на основе файла `openapi.yaml` в подпапке каждого языка. Этот слой отвечает за выполнение HTTP-запросов, маршрутизацию эндпоинтов, сериализацию/десериализацию моделей запросов и ответов.
2. **Высокоуровневая Fluent API обертка (wrapper)**: Поддерживается вручную поверх сгенерированного кода. Она предоставляет разработчикам удобный, типизированный Fluent API для описания процессов (Workflow Builder), конфигурации клиента и создания обработчиков задач (workers).

#### Типовой рабочий процесс при использовании SDK:
1. **Инициализация**: Настройка клиента с указанием URL-адреса NativeBPM и необходимых заголовков авторизации.
2. **Описание процесса (Workflow-as-Code)**: Вызов цепочки методов конструктора для описания схемы (шаги, условия перехода, пользовательские и ИИ-задачи).
3. **Развертывание и выполнение**: Отправка скомпилированного AST на сервер NativeBPM и управление запущенными инстансами.
...
// Описание и развертывание процесса
...
Инструкции по установке пакетов и примеры для конкретных языков находятся в соответствующих директориях (например, `./go`, `./typescript`, `./python`).

---

### 2. Как добавлять или изменять API
...
2. **Сгенерируйте базовые клиенты**:
   * Запустите команды генерации кода (описаны в следующем разделе), чтобы обновить сгенерированные модели и файлы во всех 10 языках.
...
### 3. Как генерировать код

Для генерации кода мы используем `openapi-generator-cli`, работающий внутри Docker-контейнеров, чтобы гарантировать одинаковый результат сборки на любых рабочих машинах.

#### Предварительные требования
* Должен быть установлен и запущен **Docker**.
* Должна быть установлена утилита **Make**.

#### Команды для генерации кода
* **Сгенерировать SDK для всех 10 языков**:
  ```bash
  make generate
  ```
* **Сгенерировать SDK для конкретного языка**:
  * **Go**: `make generate-go`
...
После генерации кода обязательно проверьте успешность сборки и тестов в директории соответствующего языка.
