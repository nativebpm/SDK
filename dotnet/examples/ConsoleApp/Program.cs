...
namespace ConsoleApp
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            try
            {
                await RunWorkflowWasmAsync((DefaultApi)api);
                await RunWorkflowWithWasmPluginsAsync((DefaultApi)api);
        ...
        static async Task RunWorkflowWasmAsync(DefaultApi api)
        {
            Console.WriteLine("--------------------------------------------------");
            ...
            workflow.Start()
                .When(V("isUrgent").Eq(true)).Then(b => {
                    b.User("reviewOrder", "Review Order Details", new Dictionary<string, object> {
            ...
            Console.WriteLine("📦 Deploying native process definition directly via JSON AST...");
            var definitionResponse = await api.DeployWorkflowAsync(workflow);
        ...
        static async Task RunWorkflowWithWasmPluginsAsync(DefaultApi api)
        {
            Console.WriteLine("--------------------------------------------------");
            ...
            workflow.Start()
                .Service("calculate", "Calculate Totals", "payment_topic", new Dictionary<string, object> {
                    { "wasmPath", "./calculate_total.wasm" }
                })
                .Ai("aiCheck", "AI Fraud Guard", new Dictionary<string, object> {
                    { "provider", "google" },
                ...
                .When(V("isFraudulent").Eq(true)).Then(b => {
                    b.User("userTask", "Manual Fraud Approval", new Dictionary<string, object> {
            ...
            Console.WriteLine("📦 Deploying WASM process definition directly via JSON AST...");
            var definitionResponse = await api.DeployWorkflowAsync(workflow);
